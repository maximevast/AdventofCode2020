rules = """bright gray bags contain 2 bright gold bags, 5 dull lavender bags.
...
for rule in rules.split("\n"):
    s = rule.replace(".", "").split(" bag contain ")
    outermost = s[0]
    content = s[1].split(", ")
    if content[0] == "no other bag":
        parsed[outermost] = {}
    else:
        parsed[outermost] = {
            c.partition(" ")[-1].replace(" bag", ""): int(c.partition(" ")[0]) for c in content
        }
...
print("PART1: ", len(can_contain))

def recurse(bag_type):
    content = parsed[bag_type]
    if not content:
        return 1
    count = 1
