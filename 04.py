...
passports = [
    {s.split(":")[0]: s.split(":")[1]
     for s in passport.replace("\n", " ").split(" ")}
    for passport in batch.split("\n\n")
]

rules = {
    "byr": [str(n) for n in range(1920, 2002+1)],
    "iyr": [str(n) for n in range(2010, 2020+1)],
    "eyr": [str(n) for n in range(2020, 2030+1)],
    "hgt": [f"{n}cm" for n in range(150, 193+1)] + [f"{n}in" for n in range(59, 76+1)],
...
for passport in passports:
    ...
    if len(required.difference(passport.keys())):
        continue

    part1_valid = part1_valid + 1

    bools = []
    for key, rule in rules.items():
        value = passport[key]
        if type(rule) == re.Pattern:
            bools.append(bool(rule.match(value)))
        else:
            bools.append(value in rule)

    part2_valid = part2_valid + int(all(bools))

print("PART1: ", part1_valid)
print("PART2: ", part2_valid)
