...
def validate_part1(pwd, rule):
    minima, maxima, letter = rule
    count = pwd.count(letter)
    return 1 if count >= minima and count <= maxima else 0

def validate_part2(pwd, rule):
    i1, i2, letter = rule
    def check_index(i):
        return pwd[i-1] == letter
    return 1 if check_index(i1) + check_index(i2) == 1 else 0

for entry in table:
    raw_rule, pwd = parse_entry(entry)
    rule = parse_rule(raw_rule)
    nb_valid_part1 = nb_valid_part1 + validate_part1(pwd, rule)
    nb_valid_part2 = nb_valid_part2 + validate_part2(pwd, rule)

print("PART1: ", (nb_valid_part1))
print("PART2: ", (nb_valid_part2))